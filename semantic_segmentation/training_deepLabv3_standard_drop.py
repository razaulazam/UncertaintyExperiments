...
from deepLabv3_standard_drop import DeepLab
...
save_model_checkpoints = curr_dir + '/deepLabv3_training_standard_drop_%.1f_%s' % (drop_rate, backbone)
log_dir_tensor_board = curr_dir + '/deepLabv3_training_logs_standard_drop_%.1f_%s' % (drop_rate, backbone)
...
for epochs in range(start_epoch, total_epochs):
    ...
    if miou > best_miou:
        ...
        summary_filename = os.path.join(curr_dir, ('summary_deepLabv3_standard_drop_%.1f_%s.txt' % (drop_rate, backbone)))
