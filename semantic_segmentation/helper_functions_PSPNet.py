...
def get_equal_interval_bins(num_bins=10):
    interval = 1.0 / num_bins
    return np.linspace(interval, 1 - interval, num_bins-1)
...
def posterior_distribution_weights(logits_probs):
    """ Function for computing the MC sample wise entropy"""
    log_prediction = logits_probs.log()
    individual_entropy = logits_probs*log_prediction
    epistemic_uncertain = torch.sum(individual_entropy, dim=1)
