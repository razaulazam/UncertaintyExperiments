...
from fine_tune_data_loader_denseNet import fine_tune_loader
from validation_data_loader_denseNet import validation_loader
from helper_functions_denseNet import heteroscedastic_loss_classification_not_vectorized, AverageMeter
from helper_functions_denseNet import poly_learning_rate, intersectionAndUnionGPU
...
save_model_checkpoints = curr_dir + '/denseNet_fine_tuning'
log_dir_tensor_board = curr_dir + '/denseNet_fine_tuning_logs'
...
fine_tune_data_loader = fine_tune_loader(data_dir=data_dir, batch_size=batch_size)
validation_data_loader = validation_loader(data_dir=data_dir, batch_size=2)

restore_from = 'denseNet_training'
...
optimizer_required = optim.SGD(net.parameters(), lr=learning_rate, weight_decay=weight_decay, momentum=momentum)
optimizer_required.load_state_dict(checkpoint['optimizer'])

for param_groups in optimizer_required.param_groups:
...
for epochs in range(start_epoch, total_epochs):
    ...
    train_loss = train(net, fine_tune_data_loader, optimizer_required, criterion_train, device, epochs)
    ...
    if miou > best_miou:
        ...
        summary_filename = os.path.join(curr_dir, 'summary_fine_tune_denseNet.txt')
...
checkpoint = {'state_dict': net.state_dict(), 'optimizer': optimizer_required.state_dict()}
