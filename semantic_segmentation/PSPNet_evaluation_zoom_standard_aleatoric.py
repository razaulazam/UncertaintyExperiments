...
from helper_functions_PSPNet import AverageMeter
from helper_functions_PSPNet import intersectionAndUnionGPU, colored_labels, compute_epistemic_uncertainty
from helper_functions_PSPNet import colored_annotations, compute_aleatoric_uncertainty, compute_aleatoric_uncertainty_class_average
from test_data_loader_PSPNet import test_loader
from helper_functions_PSPNet import get_equal_interval_bins
...
epoch_load = 229
batch_size = 4
forward_pass = 'standard_aleatoric_no_spatial'
beta_factor = 1.0
layers = 50
...
curr_dir = os.getcwd()
result_dir = 'PSPNet_%s_evaluation_%.1f' % (forward_pass, beta_factor)
tensor_dir = 'PSPNet_%s_evaluation_store_tensor_%.1f' % (forward_pass, beta_factor)
...
restore_from = 'PSPNet_training_zoom_%d_pixel_wise_50_standard_aleatoric_beta_%.1f_ND' % (zoom_factor, beta_factor)
...
net = PSPNet(layers=layers, beta_soft=beta_factor)
...
for step, (image_data, label_data) in enumerate(test_data_loader):
    with torch.no_grad():
        ...
        if zoom_factor != 8:
            logits = F.interpolate(output[:, 0, :, :, :], size=label.size()[1:], mode='bilinear',
                                   align_corners=True)
            variance = F.interpolate(output[:, 1, :, :, :], size=label.size()[1:], mode='bilinear',
                                     align_corners=True)
        else:
            logits = output[:, 0, :, :, :]
            variance = output[:, 1, :, :, :]**2
        ...
        aleatoric_uncertainty_average = compute_aleatoric_uncertainty(p, variance)
        aleatoric_uncertainty_class = compute_aleatoric_uncertainty_class_average(variance)
...
with open('mIou_PSPNet_evaluation_%s_%.1f' % (forward_pass, beta_factor), 'w') as summary:
