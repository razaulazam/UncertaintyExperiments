...
from train_data_loader_PSPNet import train_loader
from validation_data_loader_PSPNet import validation_loader
from torch.utils.tensorboard import SummaryWriter
from helper_functions_PSPNet import heteroscedastic_loss_classification_not_vectorized, AverageMeter
from helper_functions_PSPNet import poly_learning_rate, intersectionAndUnionGPU
from PSPNet_wd_aleatoric import PSPNet
...
training_name = 'beta_1.0'

data_dir = '/fs/scratch/rng_cr_bcai_dl_students/OpenData/cityscapes/'
save_model_checkpoints = curr_dir + '/PSPNet_training_zoom_8_pixel_wise_50_standard_aleatoric_%s_ND' % training_name
log_dir_tensor_board = curr_dir + '/PSPNet_training_logs_zoom_8_pixel_wise_50_standard_aleatoric_%s_ND' % training_name
...
print('>>>> BETA FACTOR = %.1f <<<<' % beta_factor)
...
net = PSPNet(layers=50, beta_soft=beta_factor)

params_list = []

modules_ori = [net.layer0, net.layer1, net.layer2, net.layer3, net.layer4]
modules_new = [net.ppm, net.cls]
...
optimizer_required = optim.SGD(params_list, lr=learning_rate, weight_decay=weight_decay, momentum=momentum)
criterion_train = heteroscedastic_loss_classification_not_vectorized

print("\nTRAINING...\n")


def train(net, train_data_loader, optimizer_required, criterion_train, device, epoch, zoom_factor):
    net.train()
    epoch_loss = 0.0
    ...
    for i, (image_data, label_data) in enumerate(train_data_loader):
        ...
        if zoom_factor != 8:
            h = int((labels.size()[1] - 1) / 8 * zoom_factor + 1)
            w = int((labels.size()[2] - 1) / 8 * zoom_factor + 1)
            # 'nearest' mode doesn't support align_corners mode and 'bilinear' mode is fine for downsampling
            labels = F.interpolate(labels.unsqueeze(1).float(), size=(h, w), mode='bilinear',
        ...
        for index in range(0, index_split):
            optimizer_required.param_groups[index]['lr'] = current_lr

        for index in range(index_split, len(optimizer_required.param_groups)):
            optimizer_required.param_groups[index]['lr'] = current_lr * 10
...
def validation(net, validation_data_loader, device, zoom_factor, ignore_index=255):
    ...
    for image_data, label_data in validation_data_loader:
        ...
        with torch.no_grad():
            ...
            if zoom_factor != 8:
                outputs = F.interpolate(outputs[:, 0, :, :, :], size=labels.size()[1:], mode='bilinear',
                                        align_corners=True)
                prediction = outputs
            else:
...
for epochs in range(start_epoch, total_epochs):
    ...
    train_loss = train(net, train_data_loader, optimizer_required, criterion_train, device, epochs, zoom_factor)
    ...
    overall_acc, class_acc, miou, iu = validation(net, validation_data_loader, device, zoom_factor)
    ...
    if miou > best_miou:
        ...
        summary_filename = os.path.join(curr_dir, ('summary_PSPNet_zoom_8_pixel_wise_standard_aleatoric_%s_ND.txt' %
                                                   training_name))
