...
class ToyExampleTransform:
    def __init__(self, patch_size, crop_size=(512, 512), ignore_label=255, padding=None):
        self.crop_h, self.crop_w = crop_size
        self.ignore_label = ignore_label
        self.black_class = 0
        self.white_class = 8
    ...
    def __call__(self, image, label):
        ...
        img_h_patch, img_w_patch = label.shape
        h_fill_patch = random.randint(0, img_h_patch - self.patch_height)
        w_fill_patch = random.randint(0, img_w_patch - self.patch_width)

        if random.random() < 0.5:
            image[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = [0, 0, 0]
            label[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = 0
        else:
            image[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = [255, 255, 255]
            label[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = 8
...
class ToyExampleTransformTest:
    def __init__(self, patch_size, crop_size=(1024, 1024), ignore_label=255, padding=None):
        self.crop_h, self.crop_w = crop_size
        self.ignore_label = ignore_label
        self.black_class = 0
        self.white_class = 8
    ...
    def __call__(self, image, label):
        ...
        if random.random() < 0.5:
            image[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = [0, 0,
                                                                                                                  0]
            label[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = 0
        else:
            image[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = [255,
                                                                                                                  255,
                                                                                                                  255]
            label[h_fill_patch:h_fill_patch + self.patch_height, w_fill_patch:w_fill_patch + self.patch_width] = 8
