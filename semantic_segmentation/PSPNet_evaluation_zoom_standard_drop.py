...
from helper_functions_PSPNet import AverageMeter, mc_dropout
...
batch_size = 4
mc_samples = 0
forward_pass = 'standard_no_spatial_drop'
layers = 50
drop_rate = 0.5
...
restore_from = 'PSPNet_training_zoom_%d_pixel_wise_50_standard_drop_%.1f_ND' % (zoom_factor, drop_rate)
...
for step, (image_data, label_data) in enumerate(test_data_loader):
    with torch.no_grad():
        ...
        for i in range(mc_samples):
            ...
            if zoom_factor != 8:
                logits = F.interpolate(output, size=label.size()[1:], mode='bilinear', align_corners=True)
            else:
                logits = output
