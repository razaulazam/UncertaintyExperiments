...
from helper_functions_PSPNet import loss_classification_standard, AverageMeter
from helper_functions_PSPNet import poly_learning_rate, intersectionAndUnionGPU
from PSPNet_d_baseline import PSPNet
...
save_model_checkpoints = curr_dir + '/PSPNet_training_zoom_8_pixel_wise_50_standard_drop_%.1f_ND' % drop_out_rate
log_dir_tensor_board = curr_dir + '/PSPNet_training_logs_zoom_8_pixel_wise_50_standard_drop_%.1f_ND' % drop_out_rate
...
net = PSPNet(layers=50, layer_drop=drop_rate)
...
def validation(net, validation_data_loader, device, zoom_factor, ignore_index=255):
    ...
    for image_data, label_data in validation_data_loader:
        ...
        with torch.no_grad():
            ...
            if zoom_factor != 8:
                outputs = F.interpolate(outputs, size=labels.size()[1:], mode='bilinear',
                                        align_corners=True)
                prediction = outputs
            else:
                prediction = outputs
...
for epochs in range(start_epoch, total_epochs):
    ...
    if miou > best_miou:
        ...
        summary_filename = os.path.join(curr_dir, ('summary_PSPNet_zoom_8_pixel_wise_50_standard_drop_%.1f.txt_ND' % drop_rate))
