...
from transform_MNIST import TrainTransform, TestTransform, TransformDataSet, ProduceMixUpAleatoricSoftLabels
from torch.utils.tensorboard import SummaryWriter
from torch.utils.data import random_split
from torch.utils.data import DataLoader
from LeNetAleatoricEpistemicMNIST import LeNet5
from helper_functions_MNIST import display_batch_images, calculate_accuracy_one_hot, calculate_accuracy, heteroscedastic_loss_classification_one_hot_labels_MN
...
save_model_checkpoints = curr_dir + '/training_LeNet5_standard_aleatoric_epistemic_%.1f_%s_soft_mix_5_7_test' % (drop_rate, name_color)
save_tensorboard_logs = curr_dir + '/training_LeNet5_standard_aleatoric_epistemic_%.1f_%s_soft_mix_5_7_test_logs' % (drop_rate, name_color)
...
# 1 - Horizontal MixUp, 2 - Vertical MixUp

data_training = ProduceMixUpAleatoricSoftLabels(data_training, 5, 7, [5, 7], 1, TrainTransform(mean, std))
...
criterion = heteroscedastic_loss_classification_one_hot_labels_MN()
...
def train():
    ...
    for data in training_data_loader:
        ...
        epoch_accuracy = calculate_accuracy_one_hot(pred=prob_output, label_true=label)
...
for epoch in range(total_epochs):
    ...
    if validation_accuracy > best_accuracy:
        ...
        with open('metrics_model_aleatoric_epistemic_%.1f_soft_mix_5_7_test.txt' % drop_rate, 'w+') as f:
