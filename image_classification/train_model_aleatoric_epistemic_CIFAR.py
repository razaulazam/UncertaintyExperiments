...
from transform_CIFAR import TrainTransform, TestTransform, ProduceMixUpAleatoricConfusedHard
from torch.utils.tensorboard import SummaryWriter
from torch.utils.data import DataLoader
from ResNet50AleatoricEpistemicCifar import ResNet50
from helper_functions_CIFAR import display_batch_images, calculate_accuracy, heteroscedastic_loss_classification_MN, adjust_learning_rate
...
drop_rate = 0.5
curr_dir = os.getcwd()
data_dir = '/fs/scratch/rng_cr_bcai_dl_students/OpenData/cifar/'
save_model_checkpoints = curr_dir + '/training_CIFAR_standard_aleatoric_epistemic_%.1f_horizontal_3_5_reg' % drop_rate
save_tensorboard_logs = curr_dir + '/training_CIFAR_standard_aleatoric_epistemic_%.1f_horizontal_3_5_reg_logs' % drop_rate


# In[4]:


data_training = CIFAR10(root=data_dir, train=True)
...
data_validation = CIFAR10(root=data_dir, train=False, transform=TestTransform())
...
data_training = ProduceMixUpAleatoricConfusedHard(data_training, 3, 5, 1, TrainTransform())
...
net = ResNet50(layer_drop=drop_rate)
...
for epoch in range(total_epochs):
    ...
    writer.add_scalar('Loss/Train', epoch, train_loss)
    writer.add_scalar('Loss/Validation', epoch, validation_loss)
    writer.add_scalar('Accuracy/Train', epoch, train_accuracy)
    writer.add_scalar('Accuracy/Validation', epoch, validation_accuracy)
    ...
    if validation_accuracy > best_accuracy:
        ...
        with open('metrics_model_aleatoric_epistemic_%.1f_cifar_horizontal_3_5_reg.txt' % drop_rate, 'w+') as f:
