...
def display_batch_images(batch_images, num_display):
    assert num_display <= batch_images.size()[0], 'Number of images to display should be equal to or less than the ' \
                                                  'batch size'
...
def calculate_accuracy_soft_labels(pred, label_true):
...
def calculate_top_k_error_test_soft_labels(output, target, topk=(1,)):
...
def heteroscedastic_loss_classification(logits_samples=500):
    def loss_function(logits_variances_pred, label_true, device):
        ...
        aleatoric_distribution = Normal(logits, variances)
...
def KLSoftLabels(logits, label):
    label_entropy = (label * (label+1e-10).log()).sum(dim=1)
    prob = F.softmax(logits, dim=1)
    ce_loss = ((prob+1e-10).log() * label).sum(dim=1)
    loss = label_entropy - ce_loss

    return loss


def heteroscedastic_loss_classification_one_hot_labels_KL(logits_samples=500):
    def loss_function(logits_variances_pred, label_true, device):
        ...
        for i in range(logits_samples):
            reparam_sample = aleatoric_distribution.rsample()
            loss += KLSoftLabels(reparam_sample, label_true).to(device)
...
def heteroscedastic_loss_classification_one_hot_labels(logits_samples=500):
    def loss_function(logits_variances_pred, label_true, device):
        loss = torch.zeros(label_true.size()[0]).float().to(device)
        logits = logits_variances_pred[:, 0, :]
        variances = logits_variances_pred[:, 1, :]
...
def adjust_learning_rate(optimizer, initial_lr, epoch):
    ...
    if epoch < 30:
        ...
        new_lr = initial_lr * (0.01 ** (epoch//30))
